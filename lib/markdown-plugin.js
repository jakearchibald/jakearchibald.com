      ...
      // Handle <script type="component">
      {
        ...
        markdownOutput = markdownOutput.replace(
          ...
          (_, inside) => {
            ...
            imports +=
              ...
              `import ${componentName}URL, { imports as ${componentName}Imports } from 'client-bundle:${modulePath}';\n`;

            extraOutput += `meta.scripts.push({ src: ${componentName}URL, imports: ${componentName}Imports, preloadOnly: true });\n`;

            // TODO NEXT:
            // Also add a script tag that will make the client render work
            // Best to add this to meta scripts - just push it - remember preloadOnly

            return (
              `<div class=\\"${containerClass}\\">" + render(h(${componentName}, ${propsJson})) + "</div>` +
              `<script type=\\"module\\">` +
              `import { h, render } from '" + preactComponentEntryURL + "';` +
              `import Component from '" + ${componentName}URL + "';` +
              `render(h(Component, ${propsJson}), document.querySelector('.${containerClass}'));` +
              `</script>`
            );
          },
        );
        ...
        if (hasComponent) {
          imports +=
            ...
            `import preactComponentEntryURL, { imports as preactComponentEntryImports } from 'client-bundle:client/preact-component-entry';\n`;
