...
// transformerNotationDiff must run before transformerIndentWrap: it inspects
// each line's original flat token structure to find `// [!code ++]` markers,
// whereas indent-wrap re-nests every line into .indent/.content cells.
const shikiTransformers = [transformerNotationDiff(), transformerIndentWrap()];
...
export function createRenderer({ shiki } = {}) {
  ...
  renderer.code = ({ text, lang }) => {
    // Strip prettier-ignore directives from the rendered output.
    text = text
      .split('\n')
      .filter((line) => {
