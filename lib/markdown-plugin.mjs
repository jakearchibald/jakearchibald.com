...
import { createRenderer, createWalkTokens } from './markdown-renderer.mjs';
...
export default function markdownPlugin() {
  return {
    ...
    async load(id) {
      ...
      // The highlighter loads languages lazily as the async walk encounters
      // each code block, so `codeToHtml` is ready by the synchronous render.
      const shiki = data.code === 'shiki' ? await getHighlighter() : undefined;

      const renderer = createRenderer({ shiki });
      const walkTokens = createWalkTokens({ shiki });
      const mdToHTML = (md) =>
        marked(md, { renderer, walkTokens, async: true });

      const html = await mdToHTML(content);

      // Bit of a hack, but assume 'summary' also needs markdown'd:
      if (data.summary) {
        data.summary = await mdToHTML(data.summary);
      }
      ...
      if (data.image) {
        imports += `import imageURL, { width as imageWidth, height as imageHeight } from 'img-url:${getFullAssetPath(
          data.image,
        )}';\n`;
        extraOutput += `meta.image = { src: imageURL, width: imageWidth, height: imageHeight };\n`;
      }
