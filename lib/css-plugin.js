...
export default function () {
  ...
  return {
    ...
    async buildStart() {
      ...
      const cssPaths = await globP('{static-build,shared}/**/*.css', {
        nodir: true,
        absolute: true,
      });
      ...
      await Promise.all(
        cssPaths.map(async (path) => {
          ...
          const cssResult = await postcss([
            ...
            postCSSSimpleVars({
              variables: {
                'mobile-boundry': '530px',
                'full-width': '1066px', // ohh battle of Hastings
                'side-introduction-width': '800px',
              },
            }),
            /*postCSSModules({
              getJSON(_, json) {
    ...
    async generateBundle(options, bundle) {
      ...
      for (const cssAsset of cssAssets) {
        bundle[cssAsset].source = bundle[cssAsset].source.replace(
          assetRe,
          (_, p1) => '/' + this.getFileName(hashToId.get(p1)),
        );
      }
